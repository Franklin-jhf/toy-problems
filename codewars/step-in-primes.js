...
function step(g, m, n) {
  return Array.from(new Array(n-m+2-(m%2===0?1:0)),(val,index)=>index+m+(m%2===0?1:0)).filter(n => n >= 2 ? Array.from(new Array(Math.floor(Math.sqrt(n) / 2)),(val,index)=>3 + index*2).every( (x) => n % x !== 0) && (n % 2 !== 0 || n === 2) : false).reduce((res,x,i,ar)=>!res?ar.map(y=>(y-x===g)?y:false).filter(z=>z)[0]:!res[1]?[res-g,res]:res,false)||null;
}