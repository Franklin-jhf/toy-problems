...
function findTheKey(messages , secrets) {
  return messages.map( (msg, scrt) => msg.split('').reduce( (res, val, i) => [...res, ((val !== secrets[scrt][i] && !res.includes(val)) &&  [val, secrets[scrt][i]].sort().join(''))], [])).reduce( (res, val) => [...res, ...val.reduce( (sum, val) => (val && !res.includes(val) && !sum.includes(val)) ? [...sum, val] : sum, [])], []).sort().join('');
}