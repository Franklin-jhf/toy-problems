...
function meeting(x, need){
  ...
  const availableChairs = x.map(([taken, total]) => Math.max(total - (`${taken}`.match(/X/g) || []).length, 0))
  if (availableChairs.reduce((acc, c) => acc + c) < need) return 'Not enough!'

  let chairsTaken = [], i = 0
  while (need > 0) {
    let chairs = availableChairs[i]
    if (chairs >= need) {
      chairsTaken.push(need)
      break
    }
    chairsTaken.push(chairs)
    need -= chairs
    i++
  }
  return chairsTaken
