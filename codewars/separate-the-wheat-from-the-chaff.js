...
function wheatFromChaff(values) {
    let indexOfLastNegative = values.length - 1
    values.forEach((n, i) => {
        while (values[indexOfLastNegative] > 0 && indexOfLastNegative > 0) {
          indexOfLastNegative--
        }
        if (n > 0 && indexOfLastNegative > i) {
            const temp = n
            values[i] = values[indexOfLastNegative]
            values[indexOfLastNegative] = temp
            indexOfLastNegative--
        }
    })
