...
function guessGifts(wishlist, presents) {
  ...
  for (let i = 0; i < wishlist.length; i++) {
    for (let j = 0; j < presents.length; j++) {
      if (wishlist[i].size === presents[j].size && wishlist[i].clatters === presents[j].clatters && wishlist[i].weight === presents[j].weight && !arr.includes(wishlist[i].name)) {arr.push(wishlist[i].name);}
    }
  }
  return arr;
}